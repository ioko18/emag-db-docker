...
###############################################################################
log(){ printf '[%s] %s\n' "$(date '+%F %T %Z')" "$*"; }
trim(){ sed -e 's/[[:space:]\r]*$//' -e 's/^[[:space:]]*//'; }
norm_bool(){ case "${1,,}" in t|true|on|1) echo t;; *) echo f;; esac; }
...
# psql în containerul DB: -X (nu ~/.psqlrc), -A/-t (unaligned/tuples only), -F '|' (separator),
# -P pager=off/footer=off ca să nu apară "(1 row)" sau pagerul.
psql_db(){
  $COMPOSE exec -T "$SERVICE_DB" \
    psql -X -A -t -F '|' -v ON_ERROR_STOP=1 -P pager=off -P footer=off \
...
psql_db -c "SELECT 'ok:db'" | sed -n '1p'
ALEMBIC_DB="$(psql_db -c "SELECT version_num FROM ${PGSCHEMA}.alembic_version" | trim || true)"
log "alembic (DB): ${ALEMBIC_DB:-n/a}"

HEADS="$(alembic_heads | sed -n 's/^Rev: \([^ ]*\).*/\1/p' | xargs)"
if [[ -n "$HEADS" ]]; then
...
read -r p_name s_name <<EOF
$(psql_db <<'SQL' | trim
WITH d AS (
  SELECT (date_trunc('month', now()) + interval '1 month')::date AS d
)
SELECT to_char(d, '"p_y"YYYY"m"MM') AS p_name,
...
[[ "${DEBUG:-}" ]] && { log "DEBUG: expected price part: $p_name"; log "DEBUG: expected stock part: $s_name"; }

price_exists="$(psql_db -c "SELECT to_regclass('${PGSCHEMA}.$p_name') IS NOT NULL" | trim)"
price_attached="$(psql_db <<SQL | trim
SELECT EXISTS (
  SELECT 1
  FROM pg_class c
  ...
  WHERE ns.nspname='${PGSCHEMA}' AND c.relname='${p_name}'
    AND nsp.nspname='${PGSCHEMA}' AND p.relname='emag_offer_prices_hist'
);
SQL
)"
stock_exists="$(psql_db -c "SELECT to_regclass('${PGSCHEMA}.$s_name') IS NOT NULL" | trim)"
stock_attached="$(psql_db <<SQL | trim
SELECT EXISTS (
  SELECT 1
  FROM pg_class c
  JOIN pg_namespace ns ON ns.oid=c.relnamespace
  ...
  JOIN pg_class p ON p.oid=i.inhparent
  JOIN pg_namespace nsp ON nsp.oid=p.relnamespace
  WHERE ns.nspname='${PGSCHEMA}' AND c.relname='${s_name}'
    AND nsp.nspname='${PGSCHEMA}' AND p.relname='emag_offer_stock_hist'
);
SQL
)"

price_exists="$(norm_bool "$price_exists")"
price_attached="$(norm_bool "$price_attached")"
stock_exists="$(norm_bool "$stock_exists")"
stock_attached="$(norm_bool "$stock_attached")"
...
WHERE np.nspname='${PGSCHEMA}' AND p.relname='emag_offer_prices_hist' AND n.nspname='${PGSCHEMA}'
ORDER BY 1;
SQL
...
JOIN pg_namespace n ON n.oid=c.relnamespace
JOIN pg_namespace np ON np.oid=p.relnamespace
WHERE np.nspname='${PGSCHEMA}' AND p.relname='emag_offer_stock_hist' AND n.nspname='${PGSCHEMA}'
ORDER BY 1;
SQL
fi
...
psql_db()   { $COMPOSE exec -T "$DB_SERVICE"  psql -v ON_ERROR_STOP=1 -U "$PGUSER" -d "$PGDATABASE" "$@"; }
psql_app()  { $COMPOSE exec -T "$APP_SERVICE" psql -v ON_ERROR_STOP=1 -U "$PGUSER" -d "$PGDATABASE" "$@"; }

ok=0; fail=0

log "health: DB reachability"
psql_db -c "SELECT current_database(), current_schema();" >/dev/null

log "alembic (DB): $(psql_db -At -c "SELECT version_num FROM ${PGSCHEMA}.alembic_version" 2>/dev/null || echo '?')"
if ! heads=$( $COMPOSE exec -T "$APP_SERVICE" alembic heads -v 2>/dev/null | sed -n '1p' ); then
  log "INFO: nu pot obține «alembic heads» din serviciul app (opțional)."
else
...
log "health: next-month partitions"
read -r p_name s_name < <(psql_db -At <<'SQL'
WITH d AS (
  SELECT (date_trunc('month', now()) + interval '1 month')::date AS d
)
SELECT
...
exists() {
  local rel="$1"
  psql_db -At <<SQL | grep -qx t
SELECT EXISTS (
  SELECT 1 FROM pg_class c
  JOIN pg_namespace n ON n.oid=c.relnamespace
  WHERE n.nspname='${PGSCHEMA}' AND c.relname='${rel}'
);
SQL
}
...
SELECT EXISTS (
  ...
  WHERE n.nspname='${PGSCHEMA}'
    AND c.relname='${child}'
    AND np.nspname='${PGSCHEMA}'
    AND p.relname='${parent}'
);
SQL
}

if exists "$p_name"; then log "OK  : price partition exists ($p_name)"; ((ok++)); else log "FAIL: price partition missing"; ((fail++)); fi
if attached_to "$p_name" "emag_offer_prices_hist"; then log "OK  : price partition attached"; ((ok++)); else log "FAIL: price partition NOT attached"; ((fail++)); fi

if exists "$s_name"; then log "OK  : stock partition exists ($s_name)"; ((ok++)); else log "FAIL: stock partition missing"; ((fail++)); fi
if attached_to "$s_name" "emag_offer_stock_hist"; then log "OK  : stock partition attached"; ((ok++)); else log "FAIL: stock partition NOT attached"; ((fail++)); fi
...
WHERE schemaname='${PGSCHEMA}'
  AND tablename IN ('mv_emag_stock_summary','mv_emag_best_offer')
ORDER BY tablename, indexname;" >/dev/null
for mv in mv_emag_stock_summary mv_emag_best_offer; do
  if psql_db -At <<SQL | grep -qx t; then
SELECT EXISTS (
  SELECT 1
  FROM pg_indexes
  WHERE schemaname='${PGSCHEMA}' AND tablename='${mv}'
    AND indexdef ILIKE '%UNIQUE INDEX % ON ${PGSCHEMA}.${mv} USING btree (offer_id)%'
);
SQL
    log "OK  : ${mv} has unique index on (offer_id)"; ((ok++))
  else
    log "FAIL: ${mv} missing unique index on (offer_id)"; ((fail++))
  fi
done

log "health: test REFRESH CONCURRENTLY (lock_timeout=3s, statement_timeout=2min)"
for mv in mv_emag_stock_summary mv_emag_best_offer; do
  if psql_db -c "SET lock_timeout='3s'; SET statement_timeout='2min'; REFRESH MATERIALIZED VIEW CONCURRENTLY ${PGSCHEMA}.${mv};" >/dev/null 2>&1; then
    log "OK  : refresh concurrently ${mv}"
    ((ok++))
  else
    log "FAIL: refresh concurrently ${mv}"
...
#!/usr/bin/env bash
set -euo pipefail
DB_URL="${DATABASE_URL:-postgresql://appuser:app@db:5432/appdb}"
psql "$DB_URL" -c "REFRESH MATERIALIZED VIEW CONCURRENTLY app.mv_best_offer_current;"
psql "$DB_URL" -c "REFRESH MATERIALIZED VIEW CONCURRENTLY app.mv_stock_summary_current;"
...
detect_base_url() {
  # 1) If BASE_URL already set, keep it
  if [ -n "${BASE_URL:-}" ]; then
    echo "$BASE_URL"
    return 0
  fi

  # 2) Try docker compose port (service 'app', container port 8001)
  if have_cmd docker; then
    if docker compose version >/dev/null 2>&1; then
      mapped="$(docker compose port app 8001 2>/dev/null || true)"
    ...
    if [ -n "$mapped" ]; then
      # formats: "0.0.0.0:8010" or "[::]:8010" or "127.0.0.1:8010"
      host_port="${mapped##*:}"
  ...
  # 3) Fallback to APP_PORT or 8001
  : "${APP_PORT:=8001}"
...
wait_for_api() {
  local url="${1}"
  local retries="${API_WAIT_RETRIES:-60}"
  local sleep_s="${API_WAIT_SLEEP_SECS:-1}"
...
json_get() {
  ...
  local jqexpr="${1:-.}"
  if have_cmd jq; then
    curl -fsS -H 'Accept: application/json' "$url" | jq -r "${jqexpr}"
  else
...
--    (Pe dataset mic, planner poate alege Seq Scan; forțăm demonstrativ:)
-- ─────────────────────────────────────────────────────────────────────────────
